    ...
    <div>
        ...
        <v-expansion-panel v-if="events.items">
            <v-expansion-panel-content v-for="event in events.items" :key="event.id">
                ...
                <div slot="header"><router-link :to="{name: 'event-detail', params: {id: event.id}}">{{event.name}}</router-link> <span v-if="event.start_date">({{event.start_date}} to {{event.end_date}})</span></div>
                {{event}}
