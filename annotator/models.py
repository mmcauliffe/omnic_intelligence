...
class Player(models.Model):
    name = models.CharField(max_length=128)
    wl_id = models.IntegerField(null=True, blank=True)

    def __str__(self):
        return '{} ({})'.format(self.name, self.wl_id)
...
class Event(models.Model):
    SPECTATOR_MODE_CHOICES = (('C', 'Color'),
                              ...
                              ('O', 'Original'))
    name = models.CharField(max_length=128)
    wl_id = models.IntegerField(null=True, blank=True, unique=True)
...
class Round(models.Model):
    ...
    def get_overtime_states(self):
        overtime_starts = self.overtimestart_set.all()
        overtime_ends = self.overtimeend_set.all()
        round_end = self.end - self.begin
        if len(overtime_starts) == 0:
            return [{'begin': 0, 'end': round_end, 'status': 'not_overtime'}]
        if len(overtime_ends) == 0:
            return [{'begin': 0, 'end': overtime_starts[0].time_point, 'status': 'not_overtime'},
                    {'begin': overtime_starts[0].time_point, 'end': round_end, 'status': 'overtime'}, ]
        states = []
        for o in zip(overtime_starts, overtime_ends):
            if not states:
                states.append({'begin': 0, 'end': o[0].time_point, 'status': 'not_overtime'})
            states.append({'begin': o[0].time_point, 'end': o[1].time_point, 'status': 'overtime'})
        if states[-1]['end'] != round_end:
            states.append({'begin': states[-1]['end'], 'end': round_end, 'status': 'not_overtime'})
        return states

    def get_pause_states(self):
        pause_starts = self.pause_set.all()
        pause_ends = self.unpause_set.all()
        round_end = self.end - self.begin
        if len(pause_starts) == 0:
            return [{'begin': 0, 'end': round_end, 'status': 'not_paused'}]
        states = []
        for o in zip(pause_starts, pause_ends):
            if not states:
                states.append({'begin': 0, 'end': o[0].time_point, 'status': 'not_paused'})
            states.append({'begin': o[0].time_point, 'end': o[1].time_point, 'status': 'paused'})
        if states[-1]['end'] != round_end:
    ...
    def get_replay_states(self):
        ...
        replay_ends = self.replayend_set.all()
        round_end = self.end - self.begin
        if len(replay_starts) == 0:
            return [{'begin': 0, 'end': round_end, 'status': 'not_replay'}]
        states = []
        for o in zip(replay_starts, replay_ends):
            if not states:
                states.append({'begin': 0, 'end': o[0].time_point, 'status': 'not_replay'})
            states.append({'begin': o[0].time_point, 'end': o[1].time_point, 'status': 'replay'})
        if states[-1]['end'] != round_end:
    ...
    def is_replay(self, time_point):
        last_replay_start = self.replaystart_set.filter(time_point__lte=time_point).order_by(
            '-time_point').first()
        if last_replay_start is None:
            return False
        last_replay_end = self.replayend_set.filter(time_point__lte=time_point).order_by(
            '-time_point').first()
        if last_replay_end is not None and last_replay_end.time_point > last_replay_start.time_point:
            return False
        return True

    def is_paused(self, time_point):
        last_pause_start = self.pause_set.filter(time_point__lte=time_point).order_by(
            '-time_point').first()
        if last_pause_start is None:
            return False
        last_pause_end = self.unpause_set.filter(time_point__lte=time_point).order_by(
            '-time_point').first()
        if last_pause_end is not None and last_pause_end.time_point > last_pause_start.time_point:
            return False
        return True

    def is_overtime(self, time_point):
        last_overtime_start = self.overtimestart_set.filter(time_point__lte=time_point).order_by(
            '-time_point').first()
        if last_overtime_start is None:
            return False
        last_overtime_end = self.overtimeend_set.filter(time_point__lte=time_point).order_by(
            '-time_point').first()
        if last_overtime_end is not None and last_overtime_end.time_point > last_overtime_start.time_point:
            return False
        return True
...
class SmallerWindowStart(models.Model):
    time_point = models.DecimalField(max_digits=6, decimal_places=1)
    round = models.ForeignKey(Round, on_delete=models.CASCADE)
...
class SmallerWindowEnd(models.Model):
    time_point = models.DecimalField(max_digits=6, decimal_places=1)
    round = models.ForeignKey(Round, on_delete=models.CASCADE)
...
class OvertimeEnd(models.Model):
    time_point = models.DecimalField(max_digits=6, decimal_places=1)
    round = models.ForeignKey(Round, on_delete=models.CASCADE)
